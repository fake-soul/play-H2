...
public class StudentController extends Controller {
    ...
    public CompletionStage<Result> indexH2() {
        return supplyAsync(() -> {
            List<Student> result = Student.find.all();
            ObjectMapper mapper = new ObjectMapper();
            JsonNode jsonData = mapper.convertValue(result, JsonNode.class);
            return ok(Util.createResponse(jsonData, true));
        }, ec.current());
    ...
    public CompletionStage<Result> createH2(Http.Request request) {
        JsonNode json = request.body().asJson();
        return supplyAsync(() -> {
            if (json == null) {
                return badRequest(Util.createResponse("Expecting Json data", false));
...
//            Optional<Student> studentOptional = studentStore.addStudent(Json.fromJson(json, Student.class));
            try {
                Json.fromJson(json, Student.class).save();
                return created(Util.createResponse("OK", true));
            }
            catch (Exception e) {
                System.out.println("Error occr");
                System.out.println(e);
                return internalServerError(Util.createResponse("Could not create data.", false));
            }
        }, ec.current());
    }

    public CompletionStage<Result> retrieveH2(int id) {
        return supplyAsync(() -> {
            final Student studentOptional = Student.find.byId(id);
//            final Optional<Student> studentOptional = studentStore.getStudent(id);
            try {
//                JsonNode jsonObjects = Json.toJson(studentOptional);

                return created(Util.createResponse(studentOptional.toString(), true));
            }
            catch (Exception e) {
                System.out.println("Error occr");
                System.out.println(e);
                return internalServerError(Util.createResponse("Could not Get data.", false));
            }
        }, ec.current());
    }
}